...
<body>
  ...
  <script>
    ...
    function showCuratedGames(currentGameFolder) {
      ...
      // Collect all games (excluding Recently Played and current game)
      const allGames = [];
      const sameCategory = [];
      ...
      // ✅ Fill 3 rows with smart mixing algorithm
      const curatedGames = [];
      const cols = 5; // Default grid columns
      const rows = 3; // 3 rows as requested
      const targetCount = cols * rows; // Always try to get 15 games
      
      // Calculate how many games we can actually get
      const maxAvailable = allGames.length;
      const actualTarget = Math.min(targetCount, maxAvailable);
      
      if (sameCategory.length === 0) {
        // No same category games, use all random
        curatedGames.push(...allGames.slice(0, actualTarget));
      } else {
        // Smart mixing: 60% same category, 40% random
        const sameCategoryTarget = Math.ceil(actualTarget * 0.6);
        ...
        // Fill remaining slots with random games from other categories
        const addedFolders = new Set(curatedGames.map(g => g.folder));
        const remainingGames = allGames.filter(g => !addedFolders.has(g.folder));
        const remainingSlots = actualTarget - curatedGames.length;
        curatedGames.push(...remainingGames.slice(0, remainingSlots));
      }
      ...
      // Display curated games - 3 rows (up to 15 games)
